...
async def main() -> None:
    gaps = ["g0E3iiu0eb401865a1a4fdf4d3fe8f87"]
    handlers = {gap: MessageHandler([r".*@.*", r".*https://.*",r".*http://.*", r".*Https://.*", r".*Http://.*"], "u0GfEVi0120e31015d717d19ea89edcd") for gap in gaps}
    async with Client(session="bote2") as client:
        @client.on_message()
